...
function check1(btn) {
  ...
  // 버튼이 클릭될 때 마다 yellow <-> pink 색상 토글 변경
  if (bgColor === "yellow") {
...
// 고전 이벤트 모델 제거
// 아이디가 test1-2인 버튼을 클릭하면
// 아이디가 test1-1인 요소의 onclick 이벤트 리스너의 이벤트 핸들러 제거
document.querySelector("#test1-2").onclick = function() {
  // 기존 onclick의 이벤트 핸들러(함수)를 null로 변경
  test1a.onclick = null;
  alert("고전 이벤트 모델 제거 완료");
}
...
// test1c 요소가 클리고디었을 때 배경색 빨간색으로 변경
